...
(async () => {
  ...
  const gymsData = [
    {
      ...
      address: "Blvd. LÇ­zaro CÇ­rdenas 2090, Lomas de Encinos, 22890 Ensenada, B.C.",
      lat: 31.9102006,
      lng: -116.5976744,
      ...
      isOpen: true,
      description: "Sucursal moderna con equipamiento de Ç§ltima generaciÇün y amplia zona de pesas libres.",
    },
    {
      id: 2,
      ...
      hours: "Abierto 24/7",
      rating: 4.6,
      services: ["Pesas Libres", "Cardio", "Ç?rea Funcional", "Vestidores", "Wi-Fi"],
      isOpen: true,
      description: "UbicaciÇün cÇ¸ntrica ideal para entrenar antes o despuÇ¸s del trabajo.",
    },
    {
      id: 3,
      name: "24/7 Fitness Santa LucÇða",
      address: "Blvd. Costero 456, Santa LucÇða, 22890 Ensenada, B.C.",
      lat: 31.8158325,
      lng: -116.5954465,
      ...
      isOpen: true,
      description: "Sucursal premium con piscina y Ç­reas de recuperaciÇün.",
    },
  ];
  ...
  function buildMenu(items) {
    ...
    navList.innerHTML = items
      .map(
        (item) =>
          `<li><a href="${item.href}" ${item.active ? 'class="active"' : ""} data-action="${
            item.action || ""
    ...
    gymsData.forEach((gym) => {
      ...
      marker.bindPopup(`
        <div class="popup-content" style="color: #fff; font-family: Outfit, sans-serif;">
          ...
          <p style="margin: 5px 0;"><strong>ÐY"?</strong> ${gym.address}</p>
          <p style="margin: 5px 0;"><strong>ÐY"z</strong> ${gym.phone}</p>
          <p style="margin: 5px 0;"><strong>ƒ?ø</strong> ${gym.hours}</p>
          <p style="margin: 5px 0;"><strong>ƒð?</strong> ${gym.rating}/5</p>
          <div style="margin-top: 10px;">
            <strong>ÐY?<‹÷? Servicios:</strong><br>
            ${gym.services.join(", ")}
          </div>
        </div>
      `);
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          L.marker([userLocation.lat, userLocation.lng], {
            ...
            .addTo(map)
            .bindPopup("Tu ubicaciÇün actual")
            .openPopup();

          calculateDistances();
          showNotification("UbicaciÇün detectada correctamente", "success");
        },
        (error) => {
          console.error("Error obteniendo ubicaciÇün:", error);
          showNotification("No se pudo obtener tu ubicaciÇün", "error");
          userLocation = { lat: 31.865, lng: -116.605 };
          calculateDistances();
        }
      );
    } else {
      showNotification("La geolocalizaciÇün no es soportada por tu navegador", "error");
      userLocation = { lat: 31.865, lng: -116.605 };
      calculateDistances();
    }
  }
