...
function bindAdminForm() {
  ...
  createForm.addEventListener("submit", async (e) => {
    ...
    const token = getAccessToken();
    if (!token) {
      setFormStatus("Necesitas iniciar sesión como ROOT.", true);
      return;
    }
...
function renderCatalog(items) {
  ...
  items.forEach((item) => {
    ...
    if (userRole === "ROOT") {
      ...
      adminChip.textContent = item.is_public ? "Público" : "Oculto";
  ...
  if (embedUrl) {
    ...
    modalBody.innerHTML = `
      <div style="padding: 1rem; color: #c7d3e4;">
        <p>Este video se abre en una nueva pestaña:</p>
...
async function deleteVideo(item) {
  ...
  if (!confirm("¿Eliminar este video del catálogo?")) return;
